...
$(BUILD):
	...
	cp $(CURDIR)/$(DIST)/$(TARGET).nds $(CURDIR)/$(DIST)/patch/$(TARGET)-r4.nds
	cp $(CURDIR)/$(DIST)/$(TARGET).nds $(CURDIR)/$(DIST)/patch/$(TARGET)-mpcf.nds
...
fat:
	@echo clean ...
	@rm -fr $(CURDIR)/drastic_dldi.img

	dd if=/dev/zero of=$(CURDIR)/drastic_dldi.img bs=1M count=1
	mkdosfs $(CURDIR)/drastic_dldi.img
